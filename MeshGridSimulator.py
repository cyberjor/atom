...
# At time 0, assume each inverter tries to match its current demand
for i in range(N):
...
def solve(load_W, I_local):
    # Step 1: initial node voltages
    V_raw = [V_NOM] * N

    # Step 2: compute surplus at each node
    surplus = [I_local[i] - load_W[i]/V_NOM for i in range(N)]

    # Step 3: current flowing between nodes
    line_I = []
    cum = 0.0
    ...
    # Step 4: effective leader current (minus import)
    ...
    # Step 5: propagate voltage drops
    V_nodes = [0.0] * N
    V_nodes[0] = V_leader
    drop_seg = []
    for i in range(1, N):
